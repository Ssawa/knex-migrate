...
function invoke(migrator, commander, env, init = true) {
...
function bootstrap(configOrFactory) {
  ...
  cli.launch({
    ...
    if (typeof(configOrFactory) == 'function') {
      migrator = configOrFactory(env, commander)
    } else {
      const knex = require(env.modulePath);
      migrator = new Migrator(knex(configOrFactory)); 
    }
    invoke(migrator, commander, env, false)
  });
}
